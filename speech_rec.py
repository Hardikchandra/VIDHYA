...
def recognize_speech_from_mic(s_flag):
    ...
    try:
        ...
        mic_index = 1  # Set to the correct index (adjust this as necessary)
        with sr.Microphone(device_index=mic_index) as source:
            ...
            if s_flag==1:
                speak_text("Listening for activation.")
            else:
                speak_text("Ask Your Question.")
            
            print("Listening for speech... Please speak something.")

            # Listen for speech with a timeout
            try:
                audio = r.listen(source, timeout=5, phrase_time_limit=10)  # Adjusted timeout and phrase time limit
